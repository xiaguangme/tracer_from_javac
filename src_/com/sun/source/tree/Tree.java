 ...
 * 这个Tree接口定义了：
 *     1. 树节点的类型(大概有好几十种)
 *     2. getKind接口，让具体的子类返回自己是什么类型
 *     3. 定于accept接口，用于访问者模式(背景知识  visitor pattern @link VisitorPattern)
 *         对于不同的节点实现Tree的时候，accept方法很容易做具体实现，一般都是vistor.visitxxx(xxx)
 *         这个参数xxx略微不同而已，比如可以看看   JCAnnotation 
 *     
 *
