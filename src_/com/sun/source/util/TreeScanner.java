...
public class TreeScanner<R,P> implements TreeVisitor<R,P> {
    ...
    public R scan(Tree node, P p) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(node, p);
	return (node == null) ? null : node.accept(this, p);
    }

    private R scanAndReduce(Tree node, P p, R r) {
         
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(node, p, r);
    ...
    public R scan(Iterable<? extends Tree> nodes, P p) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(nodes, p);
	R r = null;
        if (nodes != null) {
            boolean first = true;
            for (Tree node : nodes) {
    ...
    private R scanAndReduce(Iterable<? extends Tree> nodes, P p, R r) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(nodes, p, r);
