...
public abstract class Symbol implements Element {
    ...
    public Attribute.Compound attribute(Symbol anno) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(anno);
    ...
    public Symbol clone(Symbol newOwner) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(newOwner);
    ...
    public String location(Type site, Types types) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(site, types);
    ...
    public boolean isSubClass(Symbol base, Types types) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(base, types);
    ...
    public boolean isMemberOf(TypeSymbol clazz, Types types) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(clazz, types);
	return
            owner == clazz ||
    ...
    public boolean overrides(Symbol _other, TypeSymbol origin, Types types, boolean checkResult) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(_other, origin, types, checkResult);
	return false;
    }
    ...
    public <A extends java.lang.annotation.Annotation> A getAnnotation(Class<A> annoType) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(annoType);
	...
	if (owner == null ||
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(that, types);
	if (this == that)
                return false;
...
//          System.out.println("clone " + v + " in " + newOwner);//DEBUG
	...
	return Constants.decode(getConstValue(), type);
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(env, log, attr, initializer);
	...
	if (data == ElementKind.EXCEPTION_PARAMETER) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(data);
	...
	if ((flags() & BLOCK) != 0) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(c, types);
	Symbol impl = null;
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(_other, origin, types);
	if (isConstructor() || _other.kind != MTH) return false;
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(origin, types);
	...
	if (isConstructor() || _other.kind != MTH) return false;
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(origin);
	switch ((int)(flags_field & Flags.AccessFlags)) {
            case Flags.PRIVATE:
                return false;
            case Flags.PUBLIC:
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(origin, types, checkResult);
	...
	owner.complete();
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(index, exclude);
	String prefix = "arg";
            while (true) {
...
//          this.printStackTrace();//DEBUG
	...
	return errmsg;
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(cause);
