...
public class TransTypes extends TreeTranslator {
    ...
    JCExpression cast(JCExpression tree, Type target) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(tree, target);
	int oldpos = make.pos;
        make.at(tree.pos);
...
//      System.err.println("retype " + tree + " to " + erasedType);//DEBUG
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(tree, erasedType, target);
    ...
    <T extends JCTree> List<T> translateArgs(List<T> _args,
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(_args, parameters, varargsElement);
    ...
    void addBridge(DiagnosticPosition pos,
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pos, meth, impl, origin, hypothetical, bridges);
	make.at(pos);
    ...
    void addBridgeIfNeeded(DiagnosticPosition pos,
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pos, sym, origin, bridges);
	if (sym.kind == MTH &&
            sym.name != names.init &&
    ...
    // where
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(method, impl, dest);
	if (impl != method) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(type, method, erasure);
    ...
    void addBridges(DiagnosticPosition pos,
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pos, i, origin, bridges);
    ...
    void addBridges(DiagnosticPosition pos, ClassSymbol origin, ListBuffer<JCTree> bridges) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pos, origin, bridges);
 ...
 *************************************************************************/
    ...
    public <T extends JCTree> List<T> translate(List<T> trees, Type pt) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(trees, pt);
	Type prevPt = this.pt;
        List<T> res;
        try {
            this.pt = pt;
 ...
 *************************************************************************/
    ...
    public JCTree translateTopLevelClass(JCTree cdef, TreeMaker make) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(cdef, make);
	this.make = make;
        pt = null;
