...
public abstract class RelativePath implements Comparable<RelativePath> {
    ...
    public File getFile(File directory) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(directory);
	if (path.length() == 0)
            return directory;
    ...
    public static class RelativeFile extends RelativePath {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(className, kind);
	...
	int sep = path.lastIndexOf('/');
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(zip);
