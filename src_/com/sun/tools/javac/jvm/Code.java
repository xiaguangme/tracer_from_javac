...
/*---------- classfile fields: --------------- */
    ...
    public boolean checkLimits(DiagnosticPosition pos, Log log) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pos, log);
 ...
 ****************************************************************************/
    ...
    public static int width(int typecode) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(typecode);
	switch (typecode) {
 ...
 ****************************************************************************/
    ...
    private  void emit1(int od) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(od);
	if (!alive) return;
        if (cp == code.length) {
    ...
    private void emitop(int op) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(op);
	if (pendingJumps != null) resolvePending();
        if (alive) {
    ...
    public void emitMultianewarray(int ndims, int type, Type arrayType) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(ndims, type, arrayType);
    ...
    public void emitNewarray(int elemcode, Type arrayType) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(elemcode, arrayType);
	emitop(newarray);
        if (!alive) return;
        emit1(elemcode);
        state.pop(1); // count
    ...
    public void emitAnewarray(int od, Type arrayType) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(od, arrayType);
	emitop(anewarray);
        if (!alive) return;
        emit2(od);
        state.pop(1);
    ...
    public void emitInvokeinterface(int meth, Type mtype) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(meth, mtype);
    ...
    public void emitop1(int op, int od) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(op, od);
	emitop(op);
        if (!alive) return;
        emit1(od);
        switch (op) {
    ...
    public void emitop1w(int op, int od1, int od2) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(op, od1, od2);
    ...
    public void align(int incr) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(incr);
	if (alive)
    ...
    private void put1(int pc, int op) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pc, op);
	code[pc] = (byte)op;
    }
    ...
    private void put2(int pc, int od) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pc, od);
	put1(pc, od >> 8);
        put1(pc+1, od);
    }
    ...
    private int get1(int pc) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pc);
	return code[pc] & 0xFF;
    }
    ...
    public int entryPoint(State state) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(state);
	int pc = curPc();
        alive = true;
    ...
    public int entryPoint(State state, Type pushed) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(state, pushed);
	int pc = curPc();
        alive = true;
        this.state = state.dup();
        assert state.stacksize <= max_stack;
 ...
 *************************************************************************/
    ...
    void emitCLDCStackMap(int pc, int localsSize) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pc, localsSize);
 ...
 *************************************************************************/
    ...
    public void resolve(Chain chain, int target) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(chain, target);
    ...
    public void resolve(Chain chain) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(chain);
	assert
            !alive ||
            chain==null ||
    ...
    public static Chain mergeChains(Chain chain1, Chain chain2) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(chain1, chain2);
 ...
 ****************************************************************************/
    ...
    public void addCatch(
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(startPc, endPc, handlerPc, catchType);
 ...
 ****************************************************************************/
    ...
    public void addLineNumber(char startPc, char lineNumber) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(startPc, lineNumber);
	if (lineDebugInfo) {
 ...
 ****************************************************************************/
	...
	try {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(register);
	if (locks == null) {
                locks = new int[20];
	...
	if (!alive) return;
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(old);
 ...
 ****************************************************************************/
    ...
    public void setDefined(Bits newDefined) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(newDefined);
    ...
    public void setDefined(int adr) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(adr);
	LocalVar v = lvar[adr];
        if (v == null) {
            state.defined.excl(adr);
        } else {
    ...
    void putVar(LocalVar var) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(var);
    ...
    public void endScopes(int first) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(first);
