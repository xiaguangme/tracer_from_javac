...
public class Gen extends JCTree.Visitor {
    ...
    void loadIntConst(int n) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(n);
    ...
    public static int zero(int tc) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(tc);
	switch(tc) {
    ...
    Symbol binaryQualifier(Symbol sym, Type site) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(sym, site);
	if (site.tag == ARRAY) {
    ...
    void callMethod(DiagnosticPosition pos,
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pos, site, name, argtypes, isStatic);
	Symbol msym = rs.
    ...
    private boolean isAccessSuper(JCMethodDecl enclMethod) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(enclMethod);
	return
 ...
 *************************************************************************/
    ...
    Env<GenContext> unwind(JCTree target, Env<GenContext> env) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(target, env);
 ...
 *************************************************************************/
    ...
    List<JCTree> normalizeDefs(List<JCTree> defs, ClassSymbol c) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(defs, c);
    ...
    private void checkStringConstant(DiagnosticPosition pos, Object constValue) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pos, constValue);
    ...
    void normalizeMethod(JCMethodDecl md, List<JCStatement> initCode) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(md, initCode);
 ...
 *********************************************************************/
    ...
    void implementInterfaceMethods(ClassSymbol c, ClassSymbol site) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(c, site);
    ...
    private void addAbstractMethod(ClassSymbol c,
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(c, m);
    ...
    private void adjustAbstractMethod(ClassSymbol c,
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(c, pm, im);
 ...
 *************************************************************************/
    ...
    public void genStat(JCTree tree, Env<GenContext> env, int crtFlags) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(tree, env, crtFlags);
	if (!genCrt) {
            genStat(tree, env);
            return;
        }
    ...
    public void genStats(List<JCStatement> trees, Env<GenContext> env, int crtFlags) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(trees, env, crtFlags);
	if (!genCrt) {
            genStats(trees, env);
            return;
        }
    ...
    public CondItem genCond(JCTree tree, int crtFlags) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(tree, crtFlags);
    ...
    public CondItem genCond(JCTree _tree, boolean markBranches) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(_tree, markBranches);
    ...
    public void genArgs(List<JCExpression> trees, List<Type> pts) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(trees, pts);
...
//where
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(tree, env, fatcode);
...
//      System.err.println("Generating " + meth + " in " + meth.owner); //DEBUG
    ...
    //where
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(loop, body, cond, step, testFirst);
...
//where
       ...
       static void qsort2(int[] keys, int[] values, int lo, int hi) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(keys, values, lo, hi);
	int i = lo;
            int j = hi;
    ...
    //where
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(body, catchers, env);
	int limit = code.nextreg;
            int startpc = code.curPc();
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(tree, env, startpc, endpc, gaps);
	if (startpc != endpc) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pos, startpc, endpc, handler_pc, catch_type);
	if (startpc != endpc) {
...
//where
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pos, type, ndims);
...
//where
	...
	Type t = tree.type.baseType();
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(tree, t);
	...
	callMethod(
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(lhs, rhs, operator);
 ...
 *************************************************************************/
    ...
    public boolean genClass(Env<AttrContext> env, JCClassDecl cdef) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(env, cdef);
	try {
            attrEnv = env;
