...
import org.simonme.tracer.javac.BlockType;
...
public class Parser {
	...
	Factory instance = context.get(parserFactoryKey);
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(S, keepDocComments, genEndPos);
	if (!genEndPos)
...
/* ---------- error recovery -------------- */
    ...
    private void skip(boolean stopAtImport, boolean stopAtMemberDecl, boolean stopAtIdentifier, boolean stopAtStatement) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(stopAtImport, stopAtMemberDecl, stopAtIdentifier, stopAtStatement);
	while (true) {
             switch (S.token()) {
                case SEMI:
                    S.nextToken();
    ...
    private JCErroneous syntaxError(int pos, String key, Object... arg) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pos, key, arg);
	return syntaxError(pos, null, key, arg);
    }

    private JCErroneous syntaxError(int pos, List<JCTree> errs, String key, Object... arg) {
         
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pos, errs, key, arg);
	setErrorEndPos(pos);
    ...
    private JCErroneous syntaxError(String key) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(key);
    ...
    private JCErroneous syntaxError(String key, String arg) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(key, arg);
    ...
    public void accept(Token token) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(token);
	if (S.token() == token) {
            S.nextToken();
        } else {
    ...
    void checkNoMods(long mods) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(mods);
	if (mods != 0) {
...
/* ---------- doc comments --------- */
    ...
    void attach(JCTree tree, String dc) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(tree, dc);
...
/* -------- source positions ------- */
    ...
    private void setErrorEndPos(int errPos) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(errPos);
    ...
    protected void storeEnd(JCTree tree, int endpos) { 
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(tree, endpos);
	}
...
/* ---------- parsing -------------- */
    ...
    JCExpression literal(Name prefix) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(prefix);
	int pos = S.pos();
...
//where
    ...
    JCExpression term(int newmode) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(newmode);
	int prevmode = mode;
        mode = newmode;
        JCExpression t = term();
        lastmode = mode;
    ...
    JCExpression term2Rest(JCExpression t, int minprec) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(t, minprec);
...
//where
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pos, topOp, od1, od2);
	if (topOp == INSTANCEOF) {
    ...
    JCExpression superSuffix(List<JCExpression> typeArgs, JCExpression t) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(typeArgs, t);
	S.nextToken();
    ...
    List<JCExpression> typeArgumentsOpt(int useMode) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(useMode);
	if (S.token() == LT) {
            checkGenerics();
            if ((mode & useMode) == 0 ||
                (mode & NOPARAMS) != 0) {
    ...
    private JCArrayTypeTree bracketsOptCont(JCExpression t, int pos) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(t, pos);
	accept(RBRACKET);
        t = bracketsOpt(t);
    ...
    JCExpression creator(int newpos, List<JCExpression> typeArgs) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(newpos, typeArgs);
	switch (S.token()) {
        case BYTE: case SHORT: case CHAR: case INT: case LONG: case FLOAT:
        case DOUBLE: case BOOLEAN:
            if (typeArgs == null)
    ...
    JCExpression innerCreator(int newpos, List<JCExpression> typeArgs, JCExpression encl) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(newpos, typeArgs, encl);
	JCExpression t = toP(F.at(S.pos()).Ident(ident()));
        if (S.token() == LT) {
            checkGenerics();
            t = typeArguments(t);
    ...
    JCExpression arrayCreatorRest(int newpos, JCExpression elemtype) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(newpos, elemtype);
	accept(LBRACKET);
        if (S.token() == RBRACKET) {
            accept(RBRACKET);
                                  ...
                                  JCExpression t)
    {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(newpos, encl, typeArgs, t);
	List<JCExpression> args = arguments();
    ...
    JCExpression arrayInitializer(int newpos, JCExpression t) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(newpos, t);
	accept(LBRACE);
     ...
     */
    JCBlock block(int pos, long flags, BlockType blockType, Object[] additionalArgs) {
         
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pos, flags, blockType, additionalArgs);
	accept(LBRACE);
        
        /**
         * 如果解析到方法开始出 则插入trace方法.
         * 如果需要，提供参数的变量名称并传递给trace方法,则类似 xxxTrace(arg1, arg2...);
         */
        if(BlockType.METHOD.equals(blockType)){
    ...
    public JCBlock block(BlockType blockType, Object[] additionalArgs) {
         
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(blockType, additionalArgs);
	return block(S.pos(), 0, blockType, additionalArgs);
    }
...
//todo: skip to anchor on error(?)
	...
	int pos = S.pos();
        switch (S.token()) {
        case LBRACE:
            return block(BlockType.OTHER, null);
        case IF: {
            S.nextToken();
        ...
        case TRY: {
            S.nextToken();
            JCBlock body = block(BlockType.TRY, null);
            ...
            if (S.token() == CATCH || S.token() == FINALLY) {
                ...
                if (S.token() == FINALLY) {
                    S.nextToken();
                    finalizer = block(BlockType.OTHER, null);
                }
            } else {
        ...
        case SYNCHRONIZED: {
            ...
            JCBlock body = block(BlockType.SYNCHRONIZED, null);
	...
	int pos = S.pos();
        ...
        JCVariableDecl formal =
            variableDeclaratorId(optFinal(Flags.PARAMETER),
                                 qualident());
        accept(RPAREN);
        JCBlock body = block(BlockType.CATCH_CLAUSE, null);
    ...
    <T extends ListBuffer<? super JCExpressionStatement>> T moreStatementExpressions(int pos,
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pos, first, stats);
    ...
    JCModifiers modifiersOpt(JCModifiers partial) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(partial);
                                                                         ...
                                                                         T vdefs)
    {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(mods, type, vdefs);
                                                                     ...
                                                                     T vdefs)
    {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pos, mods, type, name, reqInit, dc, vdefs);
    ...
    JCVariableDecl variableDeclarator(JCModifiers mods, JCExpression type, boolean reqInit, String dc) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(mods, type, reqInit, dc);
    ...
    JCVariableDecl variableDeclaratorRest(int pos, JCModifiers mods, JCExpression type, Name name,
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pos, mods, type, name, reqInit, dc);
    ...
    JCVariableDecl variableDeclaratorId(JCModifiers mods, JCExpression type) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(mods, type);
	int pos = S.pos();
        Name name = ident();
    ...
    JCStatement classOrInterfaceOrEnumDeclaration(JCModifiers mods, String dc) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(mods, dc);
	if (S.token() == CLASS) {
    ...
    List<JCTree> enumBody(Name enumName) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(enumName);
	accept(LBRACE);
        ListBuffer<JCTree> defs = new ListBuffer<JCTree>();
        if (S.token() == COMMA) {
            S.nextToken();
    ...
    List<JCTree> classOrInterfaceBody(Name className, boolean isInterface) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(className, isInterface);
	accept(LBRACE);
        if (S.pos() <= errorEndPos) {
            // error recovery
	...
	if (S.token() == SEMI) {
            ...
            if (S.token() == CLASS ||
                ...
                return List.<JCTree>of(block(pos, mods.flags, BlockType.CLASS_OR_INTERFACE_BODY, null));
            } else {
                pos = S.pos();
                List<JCTypeParameter> typarams = typeParametersOpt();
    ...
    JCTree methodDeclaratorRest(int pos,
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pos, mods, type, name, typarams, isInterface, isVoid, dc);
	List<JCVariableDecl> params = formalParameters();
        ...
        if (S.token() == LBRACE) {
            ...
            if (S.pos() <= errorEndPos) {
                ...
                skip(false, true, false, false);
                if (S.token() == LBRACE) {
                    body = block(BlockType.METHOD, null);
                }
            }
        }
        JCMethodDecl result =
