...
public class Scanner implements Lexer {
	...
	Factory instance = context.get(scannerFactoryKey);
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(input);
	if (input instanceof CharBuffer) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(input, inputLength);
    ...
    /**
     * 已经插入用于追踪分析的代码的字符的总共个数
     */
    private int count4TraceCodeChar;
    ...
    // 被修改过的源码文件的代码放在这个buf中
    private char[] buf4Modified;
    ...
    private void lexError(int pos, String key, Object... args) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(pos, key, args);
    ...
    private int digit(int base) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(base);
	char c = ch;
    ...
    private void putChar(char ch) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(ch);
	if (sp == sbuf.length) {
    ...
    private void scanHexFractionAndSuffix(boolean seendigit) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(seendigit);
	this.radix = 16;
        assert ch == '.';
        putChar(ch);
        scanChar();
    ...
    private void scanNumber(int radix) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(radix);
	this.radix = radix;
    ...
    public int count4TraceCodeChar(){
         
		org.simonme.tracer.trace.Tracer.traceMethodInvoke();
	return count4TraceCodeChar;
    }
    ...
    public char[] getRawCharacters(int beginIndex, int endIndex) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(beginIndex, endIndex);
    ...
    protected void processComment(CommentStyle style) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(style);
	if (scannerDebug)
    ...
    static int testc = 0;
    @Override
    public void insertTraceCode(String traceCode)
    {
         
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(traceCode);
	if(traceCode == null || traceCode.length() == 0)
        {
            return;
        }
        int startPos4Insert = this.pos + this.count4TraceCodeChar;
        int insertCodeCharLength = traceCode.length();
        this.count4TraceCodeChar = this.count4TraceCodeChar + insertCodeCharLength;
        int endPos4Insert = this.pos + this.count4TraceCodeChar;
