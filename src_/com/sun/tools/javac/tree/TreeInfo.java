...
public class TreeInfo {
    ...
    public static boolean isSyntheticInit(JCTree stat) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(stat);
    ...
    public static int getEndPos(JCTree tree, Map<JCTree, Integer> endPositions) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(tree, endPositions);
	if (tree == null)
            return Position.NOPOS;
	...
	return endPos; }
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(endPosTable);
    ...
    public static Env<AttrContext> scopeFor(JCTree node, JCCompilationUnit unit) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(node, unit);
    ...
    public static void setSymbol(JCTree tree, Symbol sym) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(tree, sym);
	tree = skipParens(tree);
        switch (tree.getTag()) {
        case JCTree.IDENT:
