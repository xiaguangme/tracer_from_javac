...
public class Name implements javax.lang.model.element.Name {
    ...
    private static int hashValue(byte cs[], int start, int len) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(cs, start, len);
	int h = 0;
        int off = start;
    ...
    private static boolean equals(byte[] names, int index,
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(names, index, cs, start, len);
	int i = 0;
    ...
    public static Name fromUtf(Table table, byte cs[], int start, int len) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(table, cs, start, len);
...
//              System.err.println("doubling name buffer of length + " + names.length + " to fit " + len + " bytes");//DEBUG
    ...
    public static Name fromUtf(Table table, byte cs[]) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(table, cs);
...
//          System.err.println("doubling name buffer of length " + names.length + " to fit " + len + " chars");//DEBUG
    ...
    public static Name fromString(Table table, String s) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(table, s);
    ...
    public void getBytes(byte cs[], int start) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(cs, start);
    ...
    public int indexOf(byte b) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(b);
	byte[] names = table.names;
        int i = 0;
    ...
    public boolean endsWith(Name suffix) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(suffix);
	int i = len - 1;
        int j = suffix.len - 1;
        while (j >= 0 && i >= 0 &&
    ...
    public Name append(char c, Name n) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(c, n);
    ...
    public static Name concat(Table table, Name ns[]) {
		...
		org.simonme.tracer.trace.Tracer.traceMethodInvoke(table, ns);
	int len = 0;
